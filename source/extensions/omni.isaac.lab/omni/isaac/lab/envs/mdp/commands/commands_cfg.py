...
class TrajectoryCommandCfg(CommandTermCfg):
    ...
    parallel_control_stiffness: float = 1.0
    """Scale factor to convert the heading error to angular velocity command. Defaults to 1.0."""

    perpendicular_control_stiffness: float = 1.0
    """Scale factor to convert the heading error to angular velocity command. Defaults to 1.0."""
    ...
    rel_dt: int = 5
    dt: float = 0.02
    """The number of simulator dt steps per trajectory planning node"""
    ...
    freq_min: float = 0.1
    freq_max: float = 5
    """Frequency bounds on sinusoids when trajectory is randomized"""

    alpha_min: float = -5
    alpha_max: float = 5
    """Bounds on alpha for exponentials when trajectory is randomized"""
    ...
    ranges: Ranges = MISSING
    planning_ranges: Ranges = MISSING
    """Distribution ranges for the velocity commands."""
