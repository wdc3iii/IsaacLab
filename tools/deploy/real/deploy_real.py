...
class Controller:
    def __init__(self, config: Config) -> None:
        self.config = config
        self.isaac2mujoco = config["isaac2mujoco"]
        self.mujoco2isaac = list(range(len(self.isaac2mujoco)))
        for n, i in enumerate(self.isaac2mujoco):
            self.mujoco2isaac[i] = n
        ...
        self.isaac_action = self.action.copy()
    ...
    def run(self):
        ...
        self.obs[9: 9 + self.config.num_actions] = qj_obs[self.mujoco2isaac]
        self.obs[9 + self.config.num_actions: 9 + 2 * self.config.num_actions] = dqj_obs[self.mujoco2isaac]
        self.obs[9 + 2 * self.config.num_actions: 9 + 3 * self.config.num_actions] = self.isaac_action

        # Get the action from the policy network
        obs_tensor = torch.from_numpy(self.obs).unsqueeze(0)
        self.isaac_action = self.policy(obs_tensor).detach().numpy().squeeze()
        self.action = self.isaac_action[self.isaac2mujoco]
